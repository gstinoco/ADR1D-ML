...
def _identity_limits(actual, predicted):
    """
...
def main():
    ...
    panels = [
        (axes[0, 0], "actual_effective_velocity_m_s", "predicted_effective_velocity_m_s", "Effective velocity", "m/s", "#087e8b", metrics["effective_velocity"]),
        (axes[0, 1], "actual_effective_dispersion_m2_s", "predicted_effective_dispersion_m2_s", "Effective dispersion", "m2/s", "#f28e2b", metrics["effective_dispersion"]),
    ]
    ...
    for axis, actual_name, predicted_name, title, unit, color, scores in panels:
        ...
        axis.text(
            0.04,
            0.95,
            (f"R2 = {scores['r2_physical']:.3f}\n" f"MdAPE = " f"{100 * scores['median_absolute_percentage_error']:.1f}%"),
    ...
    for index, (state, color) in enumerate(zip(state_order, colors)):
        values = predictions.loc[predictions["diagnostic_decay_state"] == state, "predicted_decay_resolvable_probability"].to_numpy(dtype=float)
        jitter = rng.uniform(-0.10, 0.10, size=len(values))

        probability_axis.scatter(np.full(len(values), index) + jitter, values, s=34, color=color, alpha=0.85, edgecolor="white", linewidth=0.4)
    probability_axis.axhline(metrics["decay_resolvability"]["decision_threshold"], color="#333333", linestyle="--", linewidth=1.2, label="Decision threshold")
    ...
    probability_axis.text(
        0.98,
        0.05,
        ("Balanced accuracy = " f"{metrics['decay_resolvability']['balanced_accuracy']:.3f}\n" f"ROC AUC = {metrics['decay_resolvability']['roc_auc']:.3f}"),
    ...
    predicted_decay = predictions.loc[resolvable, "predicted_decay_rate_if_resolvable_s_1"].to_numpy(dtype=float)

    lower, upper = _identity_limits(actual_decay, predicted_decay)

    decay_axis.scatter(actual_decay, predicted_decay, s=46, color="#b07aa1", alpha=0.9, edgecolor="white", linewidth=0.5)
    ...
    decay_axis.text(
        0.04,
        0.95,
        (f"n = {decay_scores['rows']}\n" f"R2 log10 = {decay_scores['r2_log10']:.3f}\n" f"MdAPE = " f"{100 * decay_scores['median_absolute_percentage_error']:.1f}%"),
