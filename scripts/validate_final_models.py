...
TRAINING_SCRIPT_PATH = ROOT / "scripts/train_and_evaluate_final_models.py"
...
def _assert_array_close(actual, expected, label):
    """
...
def main():
    ...
    if manifest["training_script_sha256"] != _sha256(TRAINING_SCRIPT_PATH):
        raise AssertionError("Training script hash mismatch")
    ...
    with _guarded_linear_algebra():
        velocity      = np.power(10.0, models["effective_velocity"].predict(test_base[columns["effective_parameters"]]))
        dispersion    = np.power(10.0, models["effective_dispersion"].predict(test_base[columns["effective_parameters"]]))
        probabilities = models["decay_resolvability"].predict_proba(test_decay[columns["decay_resolvability"]])
        decay_rate    = np.power(10.0, models["decay_rate_resolvable"].predict(test_decay[columns["decay_rate_resolvable"]]))
    positive_index       = int(np.where(models["decay_resolvability"].classes_ == 1)[0][0])
    positive_probability = probabilities[:, positive_index]
    predicted_resolvable = (positive_probability >= float(bundle["decision_threshold"])).astype(int)

    for label, values in {"velocity": velocity, "dispersion": dispersion, "decay probability": positive_probability, "decay rate": decay_rate}.items():
        _require_finite(values, label)

    # Compare every prediction array with the published CSV artifact.
    _assert_array_close(velocity, predictions["predicted_effective_velocity_m_s"].to_numpy(dtype=float), "velocity predictions")
    _assert_array_close(dispersion, predictions["predicted_effective_dispersion_m2_s"].to_numpy(dtype=float), "dispersion predictions")
    _assert_array_close(positive_probability, predictions["predicted_decay_resolvable_probability"].to_numpy(dtype=float), "decay probabilities")
    if not np.array_equal(predicted_resolvable, predictions["predicted_decay_resolvable"].to_numpy(dtype=int)):
        raise AssertionError("Resolvable labels do not reproduce")
    _assert_array_close(decay_rate, predictions["predicted_decay_rate_if_resolvable_s_1"].to_numpy(dtype=float), "decay-rate predictions")

    actual_velocity     = predictions["actual_effective_velocity_m_s"].to_numpy(dtype=float)
    actual_dispersion   = predictions["actual_effective_dispersion_m2_s"].to_numpy(dtype=float)
    velocity_rmse_log   = math.sqrt(mean_squared_error(np.log10(actual_velocity), np.log10(velocity)))
    dispersion_rmse_log = math.sqrt(mean_squared_error(np.log10(actual_dispersion), np.log10(dispersion)))
    # Recompute representative metrics from the reproduced arrays.
    _assert_close(velocity_rmse_log, metrics["effective_velocity"]["rmse_log10"], "velocity test RMSE")
    _assert_close(dispersion_rmse_log, metrics["effective_dispersion"]["rmse_log10"], "dispersion test RMSE")

    balanced_accuracy = balanced_accuracy_score(predictions["actual_decay_resolvable"], predicted_resolvable)
    _assert_close(balanced_accuracy, metrics["decay_resolvability"]["balanced_accuracy"], "decay resolvability test score")
    resolvable          = predictions["actual_decay_resolvable"].eq(1)
    actual_decay_log    = np.log10(predictions.loc[resolvable, "actual_decay_rate_s_1"].to_numpy(dtype=float))
    predicted_decay_log = np.log10(decay_rate[resolvable.to_numpy()])
    decay_rmse_log      = math.sqrt(mean_squared_error(actual_decay_log, predicted_decay_log))
    _assert_close(decay_rmse_log, metrics["decay_rate_conditional_on_resolvable"]["rmse_log10"], "decay-rate test RMSE")
